...
Custom per-module permissions, password reset, email delivery of invites, social OAuth app keys, field-level conflict UI, offline void/return/park/shift open, full accounting ledgers / bank reconciliation / VAT returns, branch stock transfers, prescription records, manufacturing BOM, real thermal WebUSB printers, loyalty, and SMS receipts.
